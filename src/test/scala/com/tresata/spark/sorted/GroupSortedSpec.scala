...
class GroupSortedSpec extends FunSpec {
  ...
  describe("PairRDDFunctions") {
    it("should do groupSorted on a RDD without value ordering") {
      val rdd = sc.parallelize(List((1, 2), (2, 3), (1, 3), (3, 1), (2, 1)))
      val groupSorted = rdd.groupSorted(new HashPartitioner(2), None)
    ...
    it("should do groupSorted on a RDD with value ordering") {
      val rdd = sc.parallelize(List((1, 2), (2, 3), (1, 3), (3, 1), (2, 1)))
      val groupSorted = rdd.groupSorted(new HashPartitioner(2), Some(implicitly[Ordering[Int]]))
      assert(groupSorted.map(_._1).collect === List(2, 2, 1, 1, 3)) // ordering depends on hashcode!
      assert(groupSorted.glom.collect.map(_.toList).toSet ===  Set(List((2, 1), (2, 3)), List((1, 2), (1, 3), (3, 1))))
    }

    it("should do groupSorted on a GroupSorted with no effect") {
      val rdd = sc.parallelize(List((1, 2), (2, 3), (1, 3), (3, 1), (2, 1)))
      val groupSorted = rdd.groupSorted(new HashPartitioner(2), Some(implicitly[Ordering[Int]]))
      assert(groupSorted.groupSorted(new HashPartitioner(2), Some(implicitly[Ordering[Int]])) eq groupSorted)
    }
  }
  ...
  describe("GroupSorted") {
    it("should do mapStreamByKey without value ordering") {
      ...
      val sets = rdd.groupSorted(new HashPartitioner(2), None).mapStreamByKey(iter => Iterator(iter.toSet)) // very contrived...
      assert(sets.collect.toMap ===  Map("a" -> Set(1, 3), "b" -> Set(1, 10), "c" -> Set(5)))
    }

    it("should do mapStreamByKey with value ordering") {
      val rdd = sc.parallelize(List(("a", 1), ("b", 10), ("a", 3), ("b", 1), ("c", 5)))
      val withMax = rdd.groupSorted(new HashPartitioner(2), Some(implicitly[Ordering[Int]].reverse)).mapStreamByKey{ iter =>
    ...
    it("should do foldLeftByKey with value ordering") {
      ...
      val emas = tseries.groupSorted(new HashPartitioner(2), Some(ord)).foldLeftByKey(0.0){ case (acc, TimeValue(time, value)) => 0.8 * acc + 0.2 * value }
    ...
    it("should do foldLeftByKey without value ordering") {
      ...
      val sets = rdd.groupSorted(new HashPartitioner(2), None).foldLeftByKey(Set.empty[String]){ case (set, str) => set + str }
