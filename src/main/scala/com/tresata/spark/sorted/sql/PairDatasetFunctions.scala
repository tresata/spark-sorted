...
class PairDatasetFunctions[K: Encoder, V](dataset: Dataset[(K, V)]) extends Serializable {
  ...
  def groupSort(numPartitions: Int = -1, reverse: Boolean = false, sortBy: Column => Column = x => x): GroupSortedDataset[K, V] =
    GroupSortedDataset(dataset, if (numPartitions > 0) Some(numPartitions) else None, reverse, sortBy)
}
